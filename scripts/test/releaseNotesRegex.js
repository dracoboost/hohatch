...
console.log("Test result for existing regex:", match ? match[1].trim() : "not found");

// Check the regex from .github/workflows/release.yml
const workflowRegexString = '## \[' + escapedVersion + '\] - \d{4}-\d{2}-\d{2}\n([\s\S]*?)(?=## \[|$)';
const workflowRegex = new RegExp(workflowRegexString, 'u');
const workflowMatch = changelog.match(workflowRegex);
console.log("Test result for workflow regex:", workflowMatch ? workflowMatch[1].trim() : "not found");