...
from backend.exceptions import FileSystemError, HoHatchError
...
class BackendApi:
    ...
    def __init__(self):
        logger.info("Initializing BackendApi...")
        ...
        logger.info("BackendApi initialized successfully.")
    ...
    def _ensure_texconv_exists(self):
        ...
        if not (texconv_path.is_file() and os.access(texconv_path, os.X_OK)):
            logger.warning(
                f"Texconv not found at {texconv_path}. Attempting download."
            )
            try:
                self.download_service.download_texconv()
                logger.info("Texconv downloaded successfully during initialization.")
            except HoHatchError as e:
                logger.error(f"Failed to auto-download texconv: {e.message}")

    def _handle_error(
        self, e: Exception, message: str = "An unexpected error occurred."
    ):
    ...
    def open_inject_folder(self):
        try:
            self.file_service.open_folder(
                self.image_discovery_service.get_inject_folder_path()
            )
            return {"success": True}
        except HoHatchError as e:
    ...
    def set_language(self, lang):
        if lang in ["en", "ja"]:
            ...
            return {
                "success": True,
                "message_key": "language_set_success",
                "lang": lang,
            }
    ...
    def get_image_list(self, folder_type: str, use_hash_check: bool = False):
        try:
            ...
            logger.info(
                f"Loaded {len(images)} images for folder_type='{folder_type}'."
            )
            for image in images:
                logger.debug(f"  - Loaded {folder_type} image: {image.path}")
        ...
        try:
            ...
            if not base_dir_str:
                raise FileSystemError(
                    f"Could not determine base directory for {'dump' if is_dump_image else 'inject'}"
                )
            base_dir = Path(base_dir_str)
            src = self.texconv_service.get_displayable_image(
                dds_path_str, is_dump_image, base_dir
            )
            return {"success": True, "src": src}
        except HoHatchError as e:
            return self._handle_error(
                e, f"Failed to convert {dds_path_str} for display"
            )
    ...
    def batch_download_selected_dds_as_jpg(
        self, dds_path_list: list[str], output_folder: str
    ):
        try:
    ...
    def replace_dds(
        self, target_dds_path: str, replacement_image_path: str, is_dump_image: bool
    ):
        logger.info(f"Starting DDS replacement process for target: {target_dds_path}")
        logger.info(f"Replacement image: {replacement_image_path}")
        logger.info(f"Is dump image: {is_dump_image}")
        try:
            temp_dir = self.temp_base_dir / f"replace_{Path(target_dds_path).stem}"
            logger.info(f"Using temporary directory: {temp_dir}")
            self.file_service.clean_directory(temp_dir)

            logger.info("Converting replacement image to DDS...")
            final_dds = self.texconv_service.convert_to_dds(
                replacement_image_path, str(temp_dir), Path(target_dds_path).name
            )
            logger.info(f"Successfully converted to DDS: {final_dds}")

            if is_dump_image:
                inject_folder = self.image_discovery_service.get_inject_folder_path()
                if not inject_folder:
                    logger.error("Could not determine inject folder path.")
                    raise FileSystemError("Could not determine inject folder path.")
                logger.info(f"Target inject folder: {inject_folder}")
                final_path = Path(inject_folder) / Path(final_dds).name
                final_path.parent.mkdir(parents=True, exist_ok=True)
                self.file_service.move_file(final_dds, str(final_path))

                logger.info(f"Deleting original dump image: {target_dds_path}")
                self.file_service.delete_file(target_dds_path)
            else:
                final_path = Path(target_dds_path)
                self.file_service.move_file(final_dds, str(final_path))

            logger.info(f"DDS replacement successful. Final path: {final_path}")
            return {"success": True, "output_path": str(final_path)}
        except HoHatchError as e:
            logger.error(f"Failed to replace DDS file: {e.message}")
